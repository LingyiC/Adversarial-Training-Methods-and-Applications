...
class Network_Bert:
    def __init__(self, session, input_shape, dropout=0.2, lstm_units=1024, dense_units=30):
        self.sess = session
        K.backend.set_session(self.sess)

        self.input = K.layers.Input(shape=(input_shape,))
...
def main(data, n_epochs, n_ex, ex_len, lt, pm):
    ...
    net = Network_Bert(session, embedding_weights)
...
main(data='../dataset/', n_epochs=10, n_ex=64, ex_len=400, lt='none', pm=0.02)